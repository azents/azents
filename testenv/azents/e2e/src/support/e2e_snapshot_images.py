...
def _pull_snapshot(
    ...
    *,
    ...
    for candidate_sha in request.candidate_shas:
        if request.compatibility_base_sha is not None and not _compatible_with_base(
            candidate_sha=candidate_sha,
            base_sha=request.compatibility_base_sha,
    ...
    if (
        ...
        pull_requests.append(
            SnapshotPullRequest(
                ...
                candidate_shas=tuple(candidate_shas),
                compatibility_base_sha=base_sha,
            )
        )
    ...
    if not github_token or not github_actor or not pull_requests:
        return SnapshotPreparation(
            ...
            fallback_required=any(
                ...
                and request.environment_variable not in prepared_environment
