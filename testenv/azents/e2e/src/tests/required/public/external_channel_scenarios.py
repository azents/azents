...
from azentspublicclient.models.managed_binding_list_response import (
...
class _CreatedAgent(NamedTuple):
    """Authenticated owner and one created Agent identity."""
...
class _CreatedWorkspaceAgents(NamedTuple):
    """Authenticated owner and a created Agent catalog."""
...
class _SessionBinding(NamedTuple):
    """Generated-client Session and its External Channel Binding."""

    session: Any
    binding: Any


class _SessionProjection(NamedTuple):
    """Session identity and its generated-client Channel projection."""
...
def _required(value: T | None) -> T:
    """Return one required observed value."""
    if value is None:
        raise AssertionError("Expected a non-null observed value.")
    return value
...
def _web_driver(value: object) -> WebDriver:
    """Validate the browser driver fixture."""
    if not isinstance(value, WebDriver):
        raise AssertionError("Expected a Selenium WebDriver.")
    return value
...
def test_http_admission_unknown_participant_and_approval_journey(
    ...
    def binding_projection() -> _SessionProjection | None:
        sessions = chat_api.chat_v1_list_agent_sessions(
            agent_id=agent_id,
            _headers=headers,
        )
        for session in sessions.items:
            ...
            if len(projection.items) == 1:
                return _SessionProjection(session.id, projection)
        return None

    approved_session_id, bindings = _required(
        wait_until(
            binding_projection,
            timeout=10,
            interval=0.2,
...
def test_slack_binding_response_modes_gate_and_preserve_context(
    ...
    def find_binding() -> _SessionBinding | None:
        sessions = chat_api.chat_v1_list_agent_sessions(
            agent_id=agent_id,
            _headers=headers,
        )
        for session in sessions.items:
            ...
            if len(projection.items) == 1:
                return _SessionBinding(session, projection.items[0])
        return None

    session, binding = _required(
        wait_until(
            find_binding,
            timeout=15,
            interval=0.2,
    ...
    assert disconnected_update.value.status == 404
...
def test_multi_app_workspace_management_default_and_disconnect_journey(
    ...
    assert member_error.value.status == 403
    ...
    assert foreign_error.value.status == 404
...
def test_multi_app_mention_selector_deduplicates_and_binds_open_access_route(
    ...
    def find_selected_binding() -> _SessionBinding | None:
        sessions = chat_api.chat_v1_list_agent_sessions(
    ...
    selected_session, selected_binding = _required(
        wait_until(
...
def test_provider_native_channel_work_progress_journey(
    ...
    def management_projection() -> ManagedBindingListResponse | None:
        projection = external_api.external_channel_v1_list_session_channels(
            agent_id=agent_id,
    ...
    assert waiting_counts["chat.delete"] == 1
...
def test_socket_mode_recovers_then_acknowledges_and_preserves_route(
    ...
    def socket_binding() -> _SessionBinding | None:
        sessions = chat_api.chat_v1_list_agent_sessions(
            agent_id=agent_id,
            _headers=headers,
        )
    ...
    with azents_external_channel_gateway_factory():
        ...
        def reconnect_required_connection() -> ManagedConnection | None:
            connections = external_api.external_channel_v1_list_connections(
                agent_id=agent_id,
                handle=handle,
                _headers=headers,
    ...
    socket_session, socket_channel = _required(
        wait_until(
            socket_binding,
            timeout=15,
            interval=0.2,
...
def run_connection_management_web_surface_uses_redacted_operational_state(
    public_api_client: azentspublicclient.ApiClient,
    admin_api_client: azentsadminclient.ApiClient,
    azents_public_server_url: str,
    slack_provider_fake_url: str,
...
def test_discord_gateway_message_waits_for_location_then_binds(
    ...
    def gateway_binding() -> _SessionBinding | None:
        sessions = chat_api.chat_v1_list_agent_sessions(
            agent_id=agent_id,
            _headers=headers,
        )
    ...
    with azents_external_channel_gateway_factory():
        ...
        setup_threads_custom_id = _string(
            wait_until(
...
def test_discord_unmentioned_todo_work_tracks_activity_and_typing_recovers(
    ...
    def discord_binding() -> _SessionBinding | None:
        sessions = chat_api.chat_v1_list_agent_sessions(
            agent_id=agent_id,
            _headers=headers,
        )
...
def test_discord_configured_message_durably_provisions_conversation(
    ...
    guild_id = "200000000000000011"
    bot_user_id = "300000000000000011"
    bot_role_id = "350000000000000011"
    participant_id = "600000000000000011"
    timestamp = "2026-08-10T00:00:00.000000+00:00"
    scenarios = (
        ("st", "create_thread", False, "11"),
        ("sc", "get_message", True, "12"),
    )
    author: dict[str, object] = {
        "id": participant_id,
        "username": "participant",
    ...
    def channel_id(suffix: str) -> str:
        return f"4000000000000000{suffix}"
    ...
    def provider_message(
        *,
        ...
        return {
            "id": f"{message_id_prefix}0000000000000{suffix}",
            "channel_id": channel_id(suffix),
            "guild_id": guild_id,
            "content": f"<@&{bot_role_id}> {content_prefix} {setup_action_code}",
            "timestamp": timestamp,
            "edited_timestamp": None,
            "author": author,
            "mentions": [],
    ...
    def guild_create() -> dict[str, object]:
        return {
            ...
            "channels": [
                {
                    "id": channel_id(suffix),
                    "guild_id": guild_id,
                    "type": 0,
                    "name": f"durable-provisioning-{suffix}",
                    "position": index,
                    "permission_overwrites": [],
                }
                for index, (*_, suffix) in enumerate(scenarios)
            ],
            "threads": [],
            "members": [
                {
    ...
    first_messages = tuple(
        provider_message(
            suffix=suffix,
            message_id_prefix="500",
            content_prefix="Configure durable",
            setup_action_code=setup_action_code,
        )
        for setup_action_code, _, _, suffix in scenarios
    )
    second_messages = {
        setup_action_code: provider_message(
            suffix=suffix,
            message_id_prefix="510",
            content_prefix="Provision durable",
            setup_action_code=setup_action_code,
        )
        for setup_action_code, _, _, suffix in scenarios
    }
    ...
    configure_gateway(first_messages, sequence=2)
    token, _, handle, agent_id = _create_agent(
        public_api_client,
        admin_api_client,
        azents_public_server_url,
    ...
    initial_bindings: list[tuple[str, str]] = []
    with azents_external_channel_gateway_factory():
        for setup_action_code, _, _, suffix in scenarios:
            setup_custom_id = _string(
                wait_until(
                    ...
                    timeout=30,
                    interval=0.2,
                    message="Discord provisioning setup control was not delivered",
                )
            )
            _select_discord_setup_location(
                discord_provider_fake_url=discord_provider_fake_url,
            ...
            first_session, first_binding = _required(
                wait_until(
                    lambda: active_discord_binding(
                        excluded_session_ids=observed_session_ids,
                    ),
                    timeout=30,
                    interval=0.2,
            ...
            wait_until(
                lambda session_id=first_session_id: (
                    evidence
                    if len(
                        evidence := _external_channel_input_evidence(
                            public_server_url=azents_public_server_url,
            ...
            observed_session_ids.add(first_session.id)
            initial_bindings.append((first_session.id, first_binding.id))

    for session_id, binding_id in initial_bindings:
    ...
    for index, (
        ...
        owner_ready_while_blocked,
        _,
    ) in enumerate(scenarios, start=1):
        second_message = second_messages[setup_action_code]
        second_text = _string(second_message["content"])
        configure_gateway((second_message,), sequence=10 * index)
        requests.post(
            f"{discord_provider_fake_url}/__testenv/barrier",
        ...
        with azents_external_channel_gateway_factory():
            ...
            retained = _object(
                wait_until(
                    ...
                    interval=0.2,
                    message=(
                        "Configured Discord callback did not retain one durable owner"
                    ),
                )
            )
            ...
            second_session, _ = _required(
                wait_until(
                    lambda excluded=baseline_session_ids: active_discord_binding(
                        excluded_session_ids=excluded,
                    ),
                    timeout=30,
                    interval=0.2,
                    message=(
                        "Configured Discord message did not converge on one binding"
                    ),
                )
            )
...
def test_discord_message_command_selector_and_component_journey(
    ...
    def selected_binding() -> _SessionBinding | None:
        sessions = chat_api.chat_v1_list_agent_sessions(
            agent_id=agent_ids[1],
            _headers=headers,
        )
    ...
    selected_session, selected_channel = _required(
        wait_until(
            selected_binding,
            timeout=15,
            interval=0.2,
