...
Required CI runs one Testenv module job and three credential-free application-image
E2E lanes from `testenv/azents/e2e`. Module tests use
`uv run pytest -vv ./src/module_tests`. Deterministic application-image E2E uses
`uv run pytest -vv -m "not live_external and not runtime_provider and not web_surface" ./src/tests`.
Focused Runtime Provider E2E runs selected Docker Provider journeys. Web Surface E2E
runs separately with `uv run pytest -vv -m "web_surface and not live_external and not runtime_provider" ./src/tests`
and exercises worktree-built web applications through a real browser and gateway.
...
Tests under `src/module_tests/` import and exercise Azents modules or their required
local dependencies without building an Azents application image. They may use local
Docker, HTTP, WebSocket, or storage dependencies, but must not build or start an
Azents application image or depend on browser, Runtime Provider, or external
prerequisite state. Tests under `src/tests/` are application-image verification and
must build the Azents application image before exercising that boundary.
