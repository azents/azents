...
The chart does not put secret literals in values and does not render external secret-store CRDs. Consumers normally create Kubernetes Secrets through their own platform layer and reference those names through existing-Secret values. Kubernetes Runtime Provider authentication uses a projected ServiceAccount token and does not require a Provider credential Secret or a bootstrap Job.
...
- `runtimeProviderKubernetes` Deployment, ServiceAccount, PDB, workload namespace, projected ServiceAccount control token, and split leader/workload RBAC
...
- Runtime process containment: `runtimeProviderKubernetes.processContainment.enabled`
  opts the Provider into Kubernetes Pod Profile v2 and the bundled bwrap
  preparation contract. The AppArmor profile must be installed on every eligible
  Runtime node before enabling it. `runtimeClassName` is optional; when set, the
