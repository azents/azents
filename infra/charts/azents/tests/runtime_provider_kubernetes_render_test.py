...
def test_runtime_provider_kubernetes_renders_platform_transfer_egress() -> None:
    """Exact object-storage endpoint routes reach the Provider as bounded JSON."""
    rendered = _helm_template(
        "runtimeProviderKubernetes.enabled=true",
        "runtimeProviderKubernetes.image.repository=repo/provider",
        "runtimeProviderKubernetes.image.tag=sha",
        "runtimeProviderKubernetes.runnerImage.repository=repo/runner",
        "runtimeProviderKubernetes.runnerImage.tag=sha",
        "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0].endpointHostnames[0]=objects.example.com",
    ...
    assert "AZ_RUNTIME_PROVIDER_PLATFORM_TRANSFER_EGRESS" in rendered
    assert (
        'value: "[{\\"cidrs\\":[\\"198.51.100.10/32\\"],'
        '\\"endpoint_hostnames\\":[\\"objects.example.com\\"],'
        '\\"ports\\":[443]}]"'
    ) in rendered


def test_runtime_provider_kubernetes_rejects_broad_platform_transfer_cidr() -> None:
    """Platform transfer routes cannot use broad CIDRs or a default route."""
    with pytest.raises(subprocess.CalledProcessError) as raised:
        _helm_template(
            "runtimeProviderKubernetes.enabled=true",
            "runtimeProviderKubernetes.image.repository=repo/provider",
            ...
            "runtimeProviderKubernetes.runnerImage.tag=sha",
            "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0].endpointHostnames[0]=objects.example.com",
            "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0].cidrs[0]=0.0.0.0/0",
            "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0].ports[0]=443",
        )

    assert "host routes" in raised.value.stderr.lower()
...
def test_runtime_provider_kubernetes_renders_platform_transfer_egress_payload() -> None:
    """Workspace transfer routes render as the Provider's typed snake-case payload."""
    rendered = _helm_template(
        "runtimeProviderKubernetes.enabled=true",
        "runtimeProviderKubernetes.image.repository=repo/provider",
        "runtimeProviderKubernetes.image.tag=sha",
        ...
        "runtimeProviderKubernetes.runnerImage.tag=sha",
        (
            "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0]."
            "endpointHostnames[0]=objects.example.com"
        ),
        (
            "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0]."
            "cidrs[0]=198.51.100.10/32"
        ),
        (
            "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0]."
            "ports[0]=443"
    ...
    assert (
        ...
        '              value: "[{\\"cidrs\\":[\\"198.51.100.10/32\\"],'
        '\\"endpoint_hostnames\\":[\\"objects.example.com\\"],\\"ports\\":[443]}]"'
    ) in rendered


def test_runtime_provider_kubernetes_rejects_non_host_platform_transfer_route() -> None:
    """Chart rendering rejects broad object-storage routes before deployment."""
    with pytest.raises(subprocess.CalledProcessError) as raised:
        _helm_template(
            "runtimeProviderKubernetes.enabled=true",
            "runtimeProviderKubernetes.image.repository=repo/provider",
...
def test_runtime_provider_kubernetes_requires_platform_transfer_egress_for_workspace_upload() -> (
    None
):
    """Workspace Upload requires an explicit Provider route when deployed."""
    with pytest.raises(subprocess.CalledProcessError) as raised:
        _helm_template(
    ...
    assert "platformtransferegress is required" in raised.value.stderr.lower()


def test_runtime_provider_kubernetes_requires_public_endpoint_hostname_route() -> None:
    """Provider routes must identify the configured public S3 hostname."""
    with pytest.raises(subprocess.CalledProcessError) as raised:
        _helm_template(
            "server.runtimeControl.enabled=true",
            "server.runtimeControl.runnerImage.repository=repo/runner",
            ...
            "runtimeProviderKubernetes.runnerImage.repository=repo/runner",
            "runtimeProviderKubernetes.runnerImage.tag=sha",
            "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0].endpointHostnames[0]=other.example.com",
            "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0].cidrs[0]=198.51.100.10/32",
            "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0].ports[0]=443",
        )

    assert "must include the public s3 endpoint hostname" in raised.value.stderr.lower()


def test_runtime_provider_kubernetes_requires_public_endpoint_effective_port() -> None:
    """Provider routes must allow the public S3 endpoint's effective port."""
    with pytest.raises(subprocess.CalledProcessError) as raised:
        _helm_template(
            "server.runtimeControl.enabled=true",
            "server.runtimeControl.runnerImage.repository=repo/runner",
            ...
            f"server.runtimeControl.runnerImage.digest={_RUNNER_DIGEST}",
            "objectStorage.external.endpoint=https://s3.internal",
            "objectStorage.external.publicEndpoint=https://objects.example.com:8443",
            "objectStorage.external.bucket=workspace-bucket",
            "objectStorage.external.corsOrigins[0]=http://localhost:3000",
            "runtimeProviderKubernetes.enabled=true",
            "runtimeProviderKubernetes.image.repository=repo/provider",
    ...
    assert "effective port" in raised.value.stderr.lower()


def test_runtime_provider_kubernetes_accepts_public_endpoint_explicit_port() -> None:
    """Provider routes can allow a public S3 endpoint with an explicit port."""
    rendered = _helm_template(
        "server.runtimeControl.enabled=true",
        "server.runtimeControl.runnerImage.repository=repo/runner",
        "server.runtimeControl.runnerImage.tag=sha",
        ...
        "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0].endpointHostnames[0]=objects.example.com",
        "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0].cidrs[0]=198.51.100.10/32",
        "runtimeProviderKubernetes.networkPolicy.platformTransferEgress[0].ports[0]=8443",
    )
