...
async function createSignupTokenAdminClient(
  resHeaders: Headers,
...
export const signupTokenAdminRouter = router({
  list: publicProcedure.query(async ({ ctx }) => {
    try {
      const { data } = await authV1ListSignupTokens({
        client: await createSignupTokenAdminClient(ctx.resHeaders),
        throwOnError: true,
      });
      return data;
    } catch (e) {
  ...
  create: publicProcedure
    .input(
      z.object({
        ...
        deliveryMethod: z.enum(["manual", "email"]),
      }),
    )
