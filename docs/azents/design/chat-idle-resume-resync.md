...
created: 2026-06-10
updated: 2026-06-10
tags: [frontend, chat, reliability]
---
...
Base decision remains [ADR-0053](../adr/0053-chat-session-resync-scroll-state.md). ADR-0053 is an implemented decision log, so do not modify it. This document is implementation reinforcement design for that decision; after completion, current behavior is promoted to `spec/flow/chat-session-resync.md`.
...
| REQ-4 | When health check ack arrives, execute REST baseline refetch path. | ADR-0053-D4 extension |
| REQ-5 | If health check ack fails or times out, switch to ticket refresh/reconnect path. | ADR-0053-D4 |
