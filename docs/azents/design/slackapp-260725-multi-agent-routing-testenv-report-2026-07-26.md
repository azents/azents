...
a local environment blocker. Required GitHub deterministic E2E remains the executable product-evidence
lane for this branch.
...
| --- | --- | --- |
| `REQ-3`, `REQ-12` — legacy dedicated connection becomes Single without behavioral loss | Existing signed HTTP admission, connection lifecycle, Socket Mode, file, and work journeys; 13 migration cases cover legacy classification and downgrade safety. | Backend regression suite passed; Docker-backed migration/product E2E awaits CI. |
| `REQ-1`, `REQ-4` — Workspace authority and zero/populated Multi Apps | New management journey uses Owner, Manager, and Member identities; verifies Member denial, zero-Agent creation, route growth, redaction, and historical disconnect. | Collected; local execution Docker-blocked; CI required. |
| `REQ-2` — many-to-many App/Agent catalog without duplicate association | Management journey adds two Agents, repeats one association idempotently, creates a second Multi App, and associates the same Agent with both Apps. | Collected; local execution Docker-blocked; CI required. |
| `REQ-5`, `REQ-6` — selector catalog, source retention, and single selection | Interaction/selector/shortcut-source service tests cover signed scope, paging/search, stale scope, catalog access, and retained source continuation. New E2E drives a real modal and approval continuation. | Service tests passed; product E2E awaits CI. File-bearing selector continuation remains covered below the product-E2E layer by source/file service tests. |
| `REQ-7` — channel default management and handoff authority | Management E2E sets, conflicts, invalidates, lists, and clears/default-related state through generation-fenced Workspace APIs; PR 7 Web tests cover management state. | API journey awaits CI; authenticated Slack-to-Web handoff remains covered by focused API/Web tests rather than a new browser E2E in this phase. |
| `REQ-8` — mention without default selects but does not create a default | New signed Events API mention, duplicate block action, modal, and submission journey starts with an empty default list and reaches one selected binding. | Collected; local execution Docker-blocked; CI required. |
| `REQ-9` — competing resolution creates one binding and Session | Repository/service lock-order, idempotency, stale-generation, and duplicate-callback tests run in the backend suite; the new E2E repeats event, block action, submission, and decision. | Backend passed; end-to-end duplicate path awaits CI. |
| `REQ-10` — access-required selection waits for Allow and releases source once | New E2E asserts pending approval has no Session, preserves source text, repeats the decision safely, then observes one active selected binding. | Collected; local execution Docker-blocked; CI required. |
| `REQ-11` — Multi association removal preserves App/other routes; Single removal disconnects | New management E2E verifies impact preview, route removal, surviving route, invalidated default, and terminal Multi disconnect; existing dedicated lifecycle tests cover Single disconnect. | Backend passed; Multi API journey awaits CI. |
| `REQ-13` — cross-scope and stale payloads fail closed | Management, selector, interaction, admission, and repository tests cover cross-Workspace IDs, cross-resource admissions, metadata tampering, removed routes, and stale generations. | Backend passed; management E2E includes a cross-Workspace not-found assertion. |
| `REQ-14` — Agent identity presentation and safe icon fallback | Presentation and delivery unit tests cover required leading Agent name and capability-aware icon projection across provider output types. | Backend passed; existing work/file journeys remain the product-level candidates. |
| `ADR-D7` — rollout gate | Configuration and management tests keep Multi creation disabled by default; the E2E server alone opts in explicitly. | Backend passed; deterministic E2E fixture opts in by construction. |
...
- The 13 migration integration cases collect but require Docker-backed PostgreSQL to execute. The full
  backend run skipped them under the existing environment guard; GitHub Python CI is the required
  execution evidence.
...
PR 8 is merge-ready only after required GitHub checks execute the Docker-backed Python/migration and
deterministic E2E lanes successfully. If CI finds a product or fixture defect, fix it on the
responsible branch and rebase descendants. A local Docker blocker, successful collection, or green
static analysis cannot substitute for those checks.
