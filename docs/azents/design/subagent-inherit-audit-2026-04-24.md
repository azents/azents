...
- **Design documents**: [`docs/azents/design/subagent-inherit.md`](./subagent-inherit.md) (final design after review #2976), [`subagent-inherit-plan.md`](./subagent-inherit-plan.md)
- **Spec documents**:
  - [`docs/azents/spec/domain/agent.md`](../spec/domain/agent.md) (spec_version 2)
  - [`docs/azents/spec/domain/toolkit.md`](../spec/domain/toolkit.md) (spec_version 3)
  - [`docs/azents/spec/flow/subagent-delegation.md`](../spec/flow/subagent-delegation.md) (spec_version 2)
...
| SHA | Title |
|---|---|
| `a425d12f4` | feat(azents): add AgentSubagent toolkit_inherit_mode column |
| `3c106b692` | feat(azents): make llm_provider_*_id nullable for subagent model inherit |
| `18a15a4eb` | feat(azents): add subagent model pair validation ([subagent-model-pair]) |
| `c6dc8d423` | feat(azents): add toolkit_type field to ToolkitBinding |
| `4f837d294` | feat(azents): introduce resolve_invoke_input_with_model_source helper |
| `dc80e3df0` | feat(azents): branch inherit mode in subagent handler |
| `ff6bf56dd` | test(azents): unit tests for subagent inherit |
| `d52b8698a` | feat(azents-api): add toolkit_inherit_mode to agent subagent API |
| `d5bd6e9e8` | feat(azents-api): support nullable llm_provider_* in agent create/update request |
| `eb9e31fbf` | feat(azents-web): subagent toolkit/model inherit UI |
| `c9a7cdafa` | docs(azents): add model inherit rule to subagent domain spec |
| `f6efae894` | docs(azents): add main-only / inherit section to toolkit domain spec |
| `b9551fcbf` | docs(azents): reflect inherit branch in subagent-delegation flow spec |
| `94e1dd4b2` | docs(azents): add subagent inherit QA runbook |
...
|---|---|---|---|---|
| D01 | `agents.toolkit_inherit_mode VARCHAR(10) NOT NULL DEFAULT 'none'` + CHECK constraint | [`db-schemas/rdb/migrations/versions/eae41783d6f5_add_subagent_inherit.py:23-41`](../../../python/apps/azents/db-schemas/rdb/migrations/versions/eae41783d6f5_add_subagent_inherit.py) | Critical | IMPLEMENTED |
| D02 | `agents.llm_provider_*_id` DROP NOT NULL + `ck_agents_model_not_null_when_role_agent` CHECK | migration L45-68, [`rdb/models/agent.py:170-175`](../../../python/apps/azents/src/azents/rdb/models/agent.py) | Critical | IMPLEMENTED |
| D03 | `SubagentToolkitInheritMode` enum (`NONE`/`ALL`) | [`repos/agent_subagent/data.py:12-24`](../../../python/apps/azents/src/azents/repos/agent_subagent/data.py) | High | IMPLEMENTED |
| D04 | Pydantic `AgentSubagent.toolkit_inherit_mode` field + Create default `NONE` + Update TypedDict | [`repos/agent_subagent/data.py:27-63`](../../../python/apps/azents/src/azents/repos/agent_subagent/data.py) | High | IMPLEMENTED |
| D05 | `Agent.llm_provider_*_id: str \| None` in `repos/agent/data.py` | [`repos/agent/data.py:27-42`](../../../python/apps/azents/src/azents/repos/agent/data.py) | High | IMPLEMENTED |
| D06 | SQLA `RDBAgent.llm_provider_integration_id/model_id` nullable + keep FK RESTRICT | [`rdb/models/agent.py:67-80`](../../../python/apps/azents/src/azents/rdb/models/agent.py) | High | IMPLEMENTED |
| D07 | SQLA `RDBAgentSubagent.toolkit_inherit_mode` + `CK_INHERIT_MODE` constraint | [`rdb/models/agent_subagent.py:42-69`](../../../python/apps/azents/src/azents/rdb/models/agent_subagent.py) | High | IMPLEMENTED |

### 3.2 Service Layer (§"Service" + Phase B)

| # | Design requirement | Code evidence | Severity | Category |
|---|---|---|---|---|
| S01 | `InvalidModelPair` Failure type + service-layer validation | [`services/agent/data.py:274-284`](../../../python/apps/azents/src/azents/services/agent/data.py), [`services/agent/__init__.py:145-164, 393-412`](../../../python/apps/azents/src/azents/services/agent/__init__.py) | High | IMPLEMENTED |
...
| S04 | Reflect `toolkit_inherit_mode` in `services/agent_subagent/__init__.py` CRUD (create + update + response) | [`services/agent_subagent/__init__.py:108, 174`](../../../python/apps/azents/src/azents/services/agent_subagent/__init__.py), [`services/agent_subagent/data.py:22, 36`](../../../python/apps/azents/src/azents/services/agent_subagent/data.py) | High | IMPLEMENTED |
| S05 | `ParentModelUnavailable` Failure type (drift defense) | [`engine/run/input.py:78-88`](../../../python/apps/azents/src/azents/engine/run/input.py) | Medium | IMPLEMENTED |

### 3.3 Runtime (§"Runtime Implementation" + Phase B)

| # | Design requirement | Code evidence | Severity | Category |
|---|---|---|---|---|
| R01 | `resolve_invoke_input_with_model_source` helper — if `model_source_agent_id=None`, existing behavior; if value provided, model/integration based on model_agent, system_prompt/params/workspace_id based on subagent | [`engine/run/resolve.py:110-321`](../../../python/apps/azents/src/azents/engine/run/resolve.py) | Critical | IMPLEMENTED |
...
| R03 | Add `ToolkitBinding.toolkit_type: str \| None` field | [`engine/engine.py:144-162`](../../../python/apps/azents/src/azents/engine/engine.py) | High | IMPLEMENTED |
...
| R05 | `MAIN_ONLY_TOOL_NAMES` constant (tool-name level, `shell_recreate_sandbox`) — rename `_EXCLUDED_TOOL_NAMES` | [`engine/tools/subagent.py:74`](../../../python/apps/azents/src/azents/engine/tools/subagent.py) | High | IMPLEMENTED |
...
| # | Design requirement | Code evidence | Severity | Category |
|---|---|---|---|---|
| A01 | Allow `toolkit_inherit_mode` in POST/PATCH `/agents/{id}/subagents` | [`api/public/agent/v1/data.py:213, 226, 244`](../../../python/apps/azents/src/azents/api/public/agent/v1/data.py), [`api/public/agent/v1/__init__.py:691`](../../../python/apps/azents/src/azents/api/public/agent/v1/__init__.py) | High | IMPLEMENTED |
...
| A06 | Update OpenAPI spec (`specs/public/openapi.json`) | `python/apps/azents/specs/public/openapi.json` (`toolkit_inherit_mode` = 4 occurrences) | Medium | IMPLEMENTED |

### 3.5 Frontend (§"Frontend" + Phase C)

| # | Design requirement | Code evidence | Severity | Category |
|---|---|---|---|---|
| F01 | "Use parent's toolkits" checkbox on each row in `AgentSubagentSection.tsx` | [`features/agents/components/AgentSubagentSection.tsx:220-230`](../../../typescript/apps/azents-web/src/features/agents/components/AgentSubagentSection.tsx) | High | IMPLEMENTED |
...
| F03 | "Inherit model from parent" checkbox in Agent edit form (subagent role) | [`features/agents/components/AgentForm.tsx:332-349`](../../../typescript/apps/azents-web/src/features/agents/components/AgentForm.tsx) | High | IMPLEMENTED |
| F04 | Disable provider/model select + send null when inherit_model=true | `AgentForm.tsx:352-380`, [`useAgentFormContainer.ts:328-365`](../../../typescript/apps/azents-web/src/features/agents/containers/useAgentFormContainer.ts) | High | IMPLEMENTED |
| F05 | tRPC `agentSubagent.ts` — `toolkitInheritMode` create/update | [`trpc/routers/agentSubagent.ts:28, 64, 76, 103, 118`](../../../typescript/apps/azents-web/src/trpc/routers/agentSubagent.ts) | High | IMPLEMENTED |
| F06 | tRPC `agent.ts` — `llmProviderIntegrationId` / `llmProviderModelId` nullable | [`trpc/routers/agent.ts`](../../../typescript/apps/azents-web/src/trpc/routers/agent.ts) (59 lines changed) | High | IMPLEMENTED |
| F07 | Agent list/header badge ("Inherits from parent") | [`AgentHeader.tsx:102`](../../../typescript/apps/azents-web/src/features/agents/components/AgentHeader.tsx), [`AgentList.tsx:221`](../../../typescript/apps/azents-web/src/features/agents/components/AgentList.tsx) | Low | IMPLEMENTED |
| F08 | i18n — `inheritModel*` + `useParentToolkits*` keys in 4 locales en-US, ko-KR, ja-JP, fr-FR | [`messages/en-US.json`](../../../typescript/apps/azents-web/messages/en-US.json) L345-349, L390-391; same structure in ko-KR/ja-JP/fr-FR | High | IMPLEMENTED |
...
- **Location**: [`engine/run/resolve.py:360, 389`](../../../python/apps/azents/src/azents/engine/run/resolve.py)
...
`docs/azents/issues/` directory referenced by `spec/domain/agent.md:296, 720` and `spec/flow/subagent-delegation.md:270, 274, 408, 410` does not exist. This drift existed before this PR series and is unrelated to subagent-inherit. Outside this audit scope.

### 4.3 testenv scenario implementation file (carried to Phase 8)

TC1-5 + TC4-B in design §"testenv QA scenarios" were documented only as runbook; actual execution code (scenario files in `python/apps/azents-e2e/`) is written in Phase 8 (`test/subagent-inherit/testenv-qa` branch). This is as stated in Plan §"Phase 8" and outside Phase D scope. **Not MISSING.**
