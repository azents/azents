...
state, and a real empty-Valkey Runtime journey recovered automatically with a higher
durable Runner generation before completing new work.
...
| Empty-Valkey Runtime recovery | required Docker Runtime Provider E2E clears Valkey, waits for a higher Runner generation, then creates and reads a Workspace directory | `1 passed` |
...
| Enrollment rate limiting | real Redis window exhaustion, `FLUSHALL`, and fresh best-effort admission | passed |
...
ready lifecycle authority, Runner operation availability, and a successful new Agent
Workspace directory operation.
...
| `M10` | Empty-Valkey E2E proves Runtime reconnect and new work; existing durable broker, file, External Channel, and Scheduled Task recovery remains authoritative. |
| `M11` | Reference Compose Valkey has no volume and disables snapshot/AOF persistence. |
| `M12` | Migration, races, reset, stale authority, maximum string values, deployment, clients, and real E2E have deterministic coverage. |
