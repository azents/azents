...
> LLM settings are managed by [LLM Provider Integration](./llm-provider-integration.md) 3-layer model. Workspace table has no LLM-related columns.
...
BYOK settings structure for using LLM in workspace. See [LLM Provider Integration design](./llm-provider-integration.md) for details.
