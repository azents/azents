...
- `agent-session-foundation.md` describes `1 AgentRuntime = multiple AgentSessions` and exactly one active session. That document records the foundation phase and is no longer the target architecture for multi-active sessions.
