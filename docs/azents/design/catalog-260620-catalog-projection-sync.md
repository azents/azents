---
title: "Model Catalog [catalog-260620/ADR](../adr/catalog-260620-catalog-projection-sync.md) Drift Correction"
created: 2026-06-21
updated: 2026-06-21
tags: [backend, frontend, engine, documentation, historical-reconstruction]
document_role: primary
document_type: design
snapshot_id: catalog-260620
migration_source: "docs/azents/design/model-catalog-adr-0067-drift-correction.md"
historical_reconstruction: true
---

# Model Catalog [catalog-260620/ADR](../adr/catalog-260620-catalog-projection-sync.md) Drift Correction

## Context

[catalog-260620/ADR](../adr/catalog-260620-catalog-projection-sync.md) defines model catalog projection and sync as a stored projection system. The implementation that reached production created part of the persistence and API surface, but it drifted from the ADR in several user-visible and lifecycle-critical areas.
...
- ADR: `docs/azents/adr/catalog-260620-catalog-projection-sync.md`
...
[catalog-260620/ADR](../adr/catalog-260620-catalog-projection-sync.md) requires the normal picker read path to read stored projections without request-time provider listing. The current backend read path mostly follows that rule, but the web form prefetches catalog entries for every integration while rendering the form.
...
[catalog-260620/ADR-D6](../adr/catalog-260620-catalog-projection-sync.md) applies to every user/integration catalog that uses user-provided credentials. Bedrock access denied is only one example. Google Vertex AI and any future user-credential catalog provider must use the same domain failure model.
...
[catalog-260620/ADR-D16](../adr/catalog-260620-catalog-projection-sync.md)/D17 requires read APIs to return current snapshot summary, latest attempt summary, and paginated entries so the UI can distinguish never synced, syncing, failed with no snapshot, and failed with an existing snapshot.
...
[catalog-260620/ADR-D7](../adr/catalog-260620-catalog-projection-sync.md) requires an infinite-scroll model list.
...
[catalog-260620/ADR-D5](../adr/catalog-260620-catalog-projection-sync.md) requires per-integration throttling, disabled sync while running, and automatic retry suppression after credential/config failures until the user changes configuration or explicitly retries.
...
[catalog-260620/ADR-D18](../adr/catalog-260620-catalog-projection-sync.md)/D19 requires hidden reasons, diagnostics for exposed/hidden counts, and exact-only matching diagnostics for Bedrock/Vertex.
