...
issue: "#2779"
superseded_by: docs/azents/design/home-260421-home-list-2026.md
...
Reorganize azents-web workspace navigation as "agent-centric". From current flat NavLink (`Home/Members/Agents/Toolkits/Shell/Chat/Settings/Profile`) + global `/chat` structure, promote agent as primary navigation unit and place `Chat` and `Settings` as tabs under each agent.

Original design: `/tmp/design-2779/azents/project/Azents Agent-Centric Nav.html` (Claude Design bundle).
Discussion record: [`docs/azents/adr/nav-260420-nav.md`](../adr/nav-260420-nav.md).
...
| File | Lines | Usage |
|---|---|---|
| `python/apps/azents/src/azents/worker/adapters/slack.py` | 621-622 | "View in Web" button (control message create) |
| `python/apps/azents/src/azents/worker/adapters/slack.py` | 697-698 | "View in Web" button (control message update) |
| `python/apps/azents/src/azents/worker/adapters/discord.py` | 495-496 | "View in Web" button (Discord embed) |
...
```python
# testenv/azents/scenarios/agent_centric_nav_test.py

import pytest

from testenv.azents.client import AzentsClient


def test_agent_detail_chat_tab_renders_sessions(seed: AzentsClient):
...
def test_agent_settings_tab_preserves_form_values(seed: AzentsClient):
