...
erDiagram
    ...
    AGENT_SESSION ||--o| SESSION_GIT_WORKTREE : "owns generated worktree"
...
- New session creation is workspace-mode based. `existing_projects` registers explicit Project paths and does not copy Projects from the team-primary session. `git_worktree` creates an Azents-owned Git worktree from a source Project path and starting ref, then registers the created worktree path as the session Project. Legacy `project_paths` requests are treated as `existing_projects`.
...
- `POST /chat/v1/agents/{agent_id}/sessions/{session_id}/git-worktrees` attaches a new Azents-owned Git worktree Project to an existing active session. The server creates a `SessionGitWorktree` allocation, appends worktree-scoped setup steps to the session initialization queue, and returns both the allocation and the current initialization projection. The created Project appears only after the queued setup reaches the registration step and creates a `SessionWorkspaceProject` row.
...
A non-primary session can own multiple Git worktree allocations. New-session `git_worktree` mode creates the first allocation before the first run, while existing-session worktree attachment appends an additional allocation and worktree-scoped setup step group to the same session initialization queue. The queue processes worktree groups in sequence and returns initialization to `ready` only after all blocking worktree setup groups have completed.
...
- `[worktree-multiple-allocations]` — one session can own multiple `SessionGitWorktree` rows; existing-session attachment appends a new allocation instead of replacing the previous worktree.
- `[worktree-setup-queue]` — existing-session worktree setup reuses the session initialization lifecycle as a sequential queue and gates runs until blocking setup is ready.
