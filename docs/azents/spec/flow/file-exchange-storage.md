...
A Brave image-search call can materialize multiple selected thumbnails in one
client-tool result, each with its own output index and independent ModelFile
and Exchange attachment identities. Only bounded bytes fetched from the fixed
Brave thumbnail proxy host are eligible; original image and source-page URLs
remain references, not fetch targets. Image decoding and aggregate/per-file
bounds precede admission. Unavailable thumbnails remain text-only results.
This path requires no managed Runtime and does not publish files to an External
Channel implicitly.
