...
### REQ-4. One-time initial Discord title projection

The first final automatic Session title must be applied once to each connected Discord thread that Azents created for that Session and explicitly retained as awaiting its initial title.

**Acceptance criteria**

- A qualifying Discord thread eventually displays the first final automatic Session title when Discord accepts the update.
...
- Duplicate processing does not produce repeated visible rename behavior after the one-time projection has completed or been relinquished.
...
### REQ-6. System-owned asynchronous provider update
...
- The rename can complete after the title-generation task and the initial Agent turn proceed independently.
...
- Recoverable interruption does not require another Agent response or another user invocation to preserve the eligible update.
