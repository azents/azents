...
| Status/OpenAPI contract | `root` | `python/apps/azents/src/azents/api/public/agent_runtime/`, shared response/data modules, checked-in OpenAPI inputs, generated public clients, and the existing Web Runtime configuration status projection | Repository current-state projection | Revision-free desired/applied current status and direct consumer adoption without deletion UX changes | Route/model tests, OpenAPI dump and schema search, generated-client checks, Web lint/typecheck |
...
- Final validation:
  ...
  - OpenAPI dump/model tests, generated public-client checks, and existing Web status consumer lint/typecheck for current-state status;
