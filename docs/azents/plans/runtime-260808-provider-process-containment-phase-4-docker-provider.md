...
- Interfaces:
  ...
  - the trusted Runner container runs as UID/GID 0, receives no Agent-accessible
    Docker socket, drops all capabilities and adds only `CAP_SETUID`,
    `CAP_SETGID`, `CAP_SETFCAP`, and `CAP_SYS_ADMIN`, while every contained Agent
    child is UID/GID 1000 with zero capabilities, `NoNewPrivs=1`, and nested user
    namespaces denied;
...
- Independent review:
  ...
  - Criteria: conditional and honest capability advertisement; no contained-v1
    ...
    contract; restricted trusted root Runner bootstrap authority; Agent-child
