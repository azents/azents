...
- PR boundary: Session/Run-authorized internal resource operations that consume the typed ExchangeFile provenance and exact ModelFile Run lineage introduced in Phase 2
...
- Consume Phase 2's forward Alembic revision for typed ExchangeFile provenance and exact ModelFile `created_run_id` lineage; do not add a duplicate revision.
...
| --- | --- | --- | --- |
| Schema dependency | `rdb/models/exchange_file.py`, `rdb/models/model_file.py`, `db-schemas/rdb/migrations/**` | consume Phase 2 typed provenance and exact Run lineage | one migration head, model/repository tests |
