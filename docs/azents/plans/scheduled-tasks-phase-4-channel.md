...
| Cycle projection and terminal effect snapshot | `/root` | `repos/scheduled_task_cycle/data.py`, `repos/scheduled_task_cycle/__init__.py`, repository tests; `services/scheduled_task/terminal.py` and tests | Phase 3 cycle/terminal contract | Scheduled desired revision and per-part projection status/message identity; pre-delete terminal effect bundle; recovered no-effect outcome | CAS/version, coexistence, deletion-after-start, canonical-order, duplicate recovery tests |
...
| Terminal publication and exact-thread surfacing | `/root` | `engine/tools/scheduled.py`; Scheduled terminal/effect service; `slack_events.py`/Slack SDK boundary; `discord_sdk.py`, `discord_delivery.py`; focused tests | new canonical terminal commit and captured effect bundle | ordered terminal parts, Slack broadcast, Discord create-then-forward, unconditional Tracker delete, sanitized immediate outcomes | ordering, multipart, per-part failure/unknown, Tracker cleanup, crash/no-replay tests |
