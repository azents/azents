---
title: "ADR-0107: Re-Execution Preserves Model Target Intent"
created: 2026-07-10
tags: [architecture, agent, backend, chat, engine]
---

# ADR-0107: Re-Execution Preserves Model Target Intent
...
Reusing the session profile can silently change the edited or retried prompt's model. Reusing a resolved snapshot preserves the prior physical model but bypasses the current target policy and the dynamic-routing boundary established by ADR-0105.
...
Manual failed-run retry carries the failed run's original requested target and effort into a new `AgentRun`. The new run resolves that target against current Agent routing configuration under ADR-0105; it does not reuse the failed run's resolved model snapshot.
