---
title: "ADR-0169: Read Subscription Usage Through Provider Integrations"
created: 2026-07-19
tags: [architecture, backend, frontend, llm, oauth, billing, security, ux]
---

# ADR-0169: Read Subscription Usage Through Provider Integrations
...
### ADR-0169-D1. Model subscription usage as an integration-scoped live snapshot
...
### ADR-0169-D2. Support only subscription OAuth integrations
...
### ADR-0169-D3. Use provider adapters behind a normalized public contract
...
### ADR-0169-D4. Keep the feature read-through and non-durable
...
### ADR-0169-D5. Split operational and financial visibility by existing permissions
...
### ADR-0169-D6. Place the primary UI inside each LLM Settings integration card
...
### ADR-0169-D7. Treat expected provider read outcomes as typed domain state
...
### ADR-0169-D8. Respect provider-specific control planes
...
### ADR-0169-D9. Do not infer quota enforcement or remaining request count
...
- ADR-0067 remains authoritative for provider integration catalog sync. Subscription usage is intentionally not a catalog projection or stored sync lifecycle.
- ADR-0114 remains authoritative for run-scoped context usage display. Subscription usage must not be combined with its numerator or denominator.
- ADR-0165 remains authoritative for model execution provider failures. Subscription usage reads use a separate read outcome and do not create AgentRun retry state.
