---
title: "ADR-0087: Filesystem Skill Projection Revisions"
created: 2026-07-01
tags: [architecture, backend, engine, toolkit, runtime]
---
# ADR-0087: Filesystem Skill Projection Revisions
...
ADR-0086 reserves the chat input/action-message shape for a future Skill Turn Action:
...
- deterministic tool catalog and toolkit prompt rules from ADR-0085;
- `ActionMessagePayload` and a reserved `SkillAction` variant from ADR-0086.
...
### ADR-0087-D1. Use filesystem Skill packages as the canonical source
...
### ADR-0087-D2. Store Skill projections in Toolkit State, not canonical storage
...
### ADR-0087-D3. Keep runtime filesystem reads out of the session-loop read path
...
### ADR-0087-D4. Store Skill body in the projection
...
### ADR-0087-D5. Store only `latest` and `active` projection snapshots
...
### ADR-0087-D6. Create new projection revisions only at deterministic runtime-connected boundaries
...
### ADR-0087-D7. Adopt latest projection revisions only at run/turn boundaries
...
### ADR-0087-D8. Do not switch active Skill revision mid-run
...
### ADR-0087-D9. Keep prompt rendering deterministic and legacy-compatible
...
### ADR-0087-D10. Use Project boundaries as the Skill source set
...
### ADR-0087-D11. Keep Skill editing filesystem-backed
...
### ADR-0087-D12. Use `/actions` and `SkillAction` for composer integration

Skill actions should be exposed through the session-scoped action listing introduced by ADR-0086.
...
### ADR-0087-D13. Show compact Skill source labels in slash actions
...
### ADR-0087-D14. Keep the Skill toolset minimal: `load_skill` only
...
### ADR-0087-D15. Implement Skill support as a separate `SkillToolkit`
...
### ADR-0087-D16. Handle SkillAction in `InputBufferService` for MVP
...
### ADR-0087-D17. Project only `SKILL.md` content in MVP
...
### ADR-0087-D18. Keep parent and subagent Skill projections independent
...
### ADR-0087-D19. Preserve active Skill continuity through compaction prompts, not Skill state
...
### ADR-0087-D20. Reject DB-canonical Skill storage
...
### ADR-0087-D21. Reject runtime-read-on-use Skill loading
...
### ADR-0087-D22. Reject periodic or watcher-immediate Skill synchronization
