...
- ADR-0085 requires deterministic provider-facing ordering and snapshot-backed MCP tool availability.
...
- Delaying MCP `list_tools`; snapshot-backed passive discovery remains governed by ADR-0085.
...
### ADR-0147-D1. Keep one session-scoped working set; apply the limit at each prepared model call
...
### ADR-0147-D2. Treat the tool declaration limit as an optional request compatibility exception; default to unlimited

A maximum tool declaration count is not assumed to be a universal model capability. Before every prepared model call, Azents resolves an optional explicit limit from the current provider-request compatibility policy described in ADR-0147-D9. An absent resolved value means unlimited and does not trigger count-based truncation.
...
### ADR-0147-D3. Keep core execution tools direct and make attached service tools deferred
...
### ADR-0147-D4. Automatically activate a small ranked result set and update shared session recency
...
### ADR-0147-D5. Persist final model-visible tool names in session-bound Toolkit State
...
### ADR-0147-D6. Build a deterministic in-memory BM25 index from the current deferred catalog
...
### ADR-0147-D7. Keep the Tool Search schema description generic and stable
...
### ADR-0147-D8. Apply limits to the final provider-counted tool declarations
...
### ADR-0147-D9. Resolve verified limits from a versioned provider-request compatibility registry
...
Before each model call is prepared, Azents resolves the effective rule from the current request's provider, lowering mode, and runtime model identifier. The deployed compatibility registry is the runtime authority, so an older `AgentModelSelection` capability snapshot cannot freeze a stale hard limit. This call-time authority supersedes ADR-0067-D13 only for the effective tool declaration limit as a transport compatibility constraint; all other normalized model capabilities retain the existing saved-snapshot semantics. If a provider model-listing source later supplies an explicit, semantically equivalent maximum declaration count, the normalized value may be retained as catalog metadata and used only when no registry rule matches. Unverified or ambiguous source fields are ignored. If neither source supplies a verified value, the effective limit remains absent and ADR-0147-D2's unlimited behavior applies.
