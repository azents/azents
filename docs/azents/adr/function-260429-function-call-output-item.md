---
title: "Split FunctionCallItem.output into a Separate FunctionCallOutputItem"
created: 2026-04-29
tags: [architecture, engine, backend, historical-reconstruction, migration]
...
# function-260429/ADR: Split FunctionCallItem.output into a Separate FunctionCallOutputItem

## Context

This follows [events-260428/ADR](./events-260428-events-table-as-truth.md), which made the events table the single source of truth. Once the events table was defined as a row-per-event schema with `type/data/raw_data/external_id`, only `FunctionCallItem` in `SessionEvent` had an impedance mismatch:
...
The conversion layer in option A was removed in Phase 1 of the events unification stack. The schema mismatch in option C was never adopted because the events_v2 schema is tied to [events-260428/ADR](./events-260428-events-table-as-truth.md)'s row-per-event decision.

## References

- Design: [`design/function-260429-function-call-output-item.md`](../design/function-260429-function-call-output-item.md) §DP1
- [events-260428/ADR](./events-260428-events-table-as-truth.md): [`events-260428-events-table-as-truth.md`](./events-260428-events-table-as-truth.md) — events unification decision
...
- Historical source filename: `0004-split-function-call-output-item.md`
