---
title: "ADR-0035: Introduce SandboxProviderControl"
created: 2026-05-21
tags: [architecture, backend, engine, infra, security]
---

# ADR-0035: Introduce SandboxProviderControl

## Context

ADR-0008 adopted a structure where an in-sandbox client opens an outbound `SandboxControlRuntime.Connect` gRPC stream to NoIntern, and the worker requests command/file/checkpoint operations through `SandboxControlWorker`. This decision separated command/file/checkpoint transport from Kubernetes Pod IP, Docker network discovery, and inbound sidecar daemon calls.
