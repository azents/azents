---
title: "ADR-0168: Use Single Durable Events for Provider Tool Items"
created: 2026-07-19
tags: [architecture, backend, engine, frontend, llm, tools, storage]
---

# ADR-0168: Use Single Durable Events for Provider Tool Items
...
ADR-0167 introduced a shared provider-neutral `ProviderToolSemanticContent` contract for provider-hosted tools while retaining both `provider_tool_call` and `provider_tool_result` event kinds. That contract correctly allows a single native hosted-tool item to carry both semantic input and output, but the initial implementation still classifies OpenAI Responses `image_generation_call` as `provider_tool_result`.
...
ADR-0164 remains correct that a successful generated image requires two independently stored resources:
...
This decision supersedes ADR-0167-D2 only where it retains a separate provider result event kind. ADR-0167's adapter-owned semantic extraction, bounding, references, and strict native-artifact rules remain in force.
...
This changes only the event reference location from ADR-0164. Exchange and ModelFile storage identities, authorization, lifecycle, and atomic admission remain independent.
...
- [ADR-0046: File and Media Resource Lifecycle](./0046-file-media-resource-lifecycle.md)
- [ADR-0057: Event and Native Event Terminology](./0057-event-native-event-terminology.md)
- [ADR-0164: Materialize Provider-Generated Images as File Resources](./0164-materialize-provider-generated-images-as-file-resources.md)
