---
title: "ADR-0171: Archived Session Retention and Durable Purge"
created: 2026-07-19
tags: [architecture, backend, frontend, scheduler, session, retention]
---

# ADR-0171: Archived Session Retention and Durable Purge

## Context

ADR-0079 introduced AgentSession archive as a soft transition for inactive non-primary sessions. It intentionally omitted an archived-session browser and restore flow, and it did not define a retention deadline. Archived session data therefore remains indefinitely unless the existing public hard-delete API is called.

The SessionAgent tree model introduced after ADR-0079 gives a root AgentSession ownership of child and nested SessionAgent nodes and their linked child AgentSessions. A retention policy must delete this ownership tree as one unit rather than treating only the visible root session as the lifecycle boundary.
...
### ADR-0168-D1. Archived sessions use an admin-managed retention deadline
...
### ADR-0168-D2. Archive snapshots the deadline and administrators choose change scope
...
### ADR-0168-D3. Archive remains reversible until purge starts
...
### ADR-0168-D4. The root SessionAgent tree is one retention unit
...
### ADR-0168-D5. Purge is a durable scheduled workflow
...
### ADR-0168-D6. ModelFiles are deleted during purge
...
### ADR-0168-D7. Artifacts are deleted during purge
...
### ADR-0168-D8. ExchangeFiles gain a root-session retention owner
...
### ADR-0168-D9. Worktree cleanup moves from archive to purge
...
### ADR-0168-D10. Archived sessions cannot resume execution
...
- ADR-0079's initial omission of an archived-session browser and restore flow.
- ADR-0079's implicit indefinite retention of archived durable data.
- ADR-0092-D5's rule that archive initiates worktree cleanup.
- ADR-0080-D1 through D3 only where they make TTL the exclusive terminal boundary for Artifact and ExchangeFile. Ordinary TTL remains valid, while owning-session purge may delete these resources earlier.
- ADR-0046's independent ExchangeFile retention only where it excludes an earlier owning-session purge boundary.

ADR-0079's team-primary and running-session archive restrictions remain in effect. ADR-0092's explicit ownership validation remains in effect and is reused by purge. Unbound ExchangeFiles remain exclusively TTL-owned under ADR-0080.
