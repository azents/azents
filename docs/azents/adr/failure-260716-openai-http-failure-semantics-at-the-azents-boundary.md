---
title: "Keep OpenAI HTTP Failure Semantics at the Azents Boundary"
created: 2026-07-16
tags: [architecture, backend, engine, llm, openai, reliability, security, historical-reconstruction, migration]
...
# failure-260716/ADR: Keep OpenAI HTTP Failure Semantics at the Azents Boundary
...
Each physical SDK request receives the existing connect-only HTTP timeout. The SDK retry default from [sdk-260716/ADR](./sdk-260716-openai-sdk-http-retries.md) remains active. Retry and backoff time is contained within the same outer parsed-event idle and absolute attempt deadlines.
...
`asyncio.CancelledError` is never converted to a provider or model failure. It is re-raised immediately after the watchdog initiates stream cleanup. The logical operation closes its SDK client during finalization as established by [clients-260716/ADR](./clients-260716-openai-sdk-clients-to-logical-operations.md). User Stop retains priority over a concurrent watchdog expiry.
...
- Historical source filename: `0157-keep-openai-http-failure-semantics-at-the-azents-boundary.md`
