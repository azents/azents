---
title: "ADR-0127: Handle Message Edits as Transactional Preparation"
created: 2026-07-12
tags: [architecture, api, backend, database, engine, session]
---

# ADR-0127: Handle Message Edits as Transactional Preparation
...
ADR-0125 makes input-buffer processing a sequential preparation stage before turn start. An edit already requires an idle-only lock, durable history rewrite, and immediate history reload, so routing its replacement message through the buffer adds an unnecessary intermediate state.
...
- [ADR-0051: Move Chat Writes to a REST Commit Boundary](./0051-rest-chat-write-boundary.md)
- [ADR-0061: Migrate Chat Input and Control to a Clean Control Plane](./0061-input-control-plane-clean-migration.md)
