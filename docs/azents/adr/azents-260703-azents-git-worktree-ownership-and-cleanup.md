---
title: "Azents-Owned Git Worktree Ownership and Cleanup"
created: 2026-07-03
tags: [architecture, backend, runtime, git, session, historical-reconstruction, migration]
...
# azents-260703/ADR: Azents-Owned Git Worktree Ownership and Cleanup
...
### azents-260703/ADR-D1 — Separate worktree ownership from Project registration
...
### azents-260703/ADR-D2 — Create branch-backed Azents-owned worktrees by default
...
### azents-260703/ADR-D3 — Use an Azents management root but never infer ownership from path alone
...
The `{session_handle}` component is the durable `agent_sessions.handle` value defined by [initialization-260703/ADR](./initialization-260703-initialization-lifecycle.md). Worktree path and branch naming use the stored handle and never derive names from session title, prompt text, or raw UUID strings.
...
### azents-260703/ADR-D4 — Register worktree Projects only after Git setup succeeds
...
### azents-260703/ADR-D5 — Archive/delete cleanup runs asynchronously and is best-effort in the MVP
...
### azents-260703/ADR-D6 — Destructive cleanup requires explicit ownership validation
...
### azents-260703/ADR-D7 — Git worktree operations are typed runner operations
...
### azents-260703/ADR-D8 — Keep catalog metadata out of worktree ownership
...
### azents-260703/ADR-D9 — Git source/ref discovery is a separate typed preview operation
...
- Historical source filename: `0092-azents-owned-git-worktree-ownership-and-cleanup.md`
