...
created: 2026-06-27
tags: [architecture, backend, engine, runtime, toolkit, historical-reconstruction, migration]
...
---
# exec-260627/ADR: Runtime Exec Process Tools
...
### exec-260627/ADR-D1. Replace `bash` with `exec_command` and `write_stdin`
...
### exec-260627/ADR-D2. Runtime process handles are runner-owned
...
### exec-260627/ADR-D3. Process state is runner-memory only and ephemeral
...
### exec-260627/ADR-D4. Add generic `FunctionToolResult.metadata` as a JSON object
...
### exec-260627/ADR-D5. Runner-owned output uses bounded unread buffers and live deltas
...
### exec-260627/ADR-D6. Exec process lifecycle is bounded
...
### exec-260627/ADR-D7. Initial implementation is pipe-based; PTY is deferred
...
### exec-260627/ADR-D8. Exec processes are separate from engine background tool calls
...
- [background-260416/ADR: Background Tool Call Design Discussion](./background-260416-background-call.md)
...
- Historical source filename: `0081-runtime-exec-process.md`
