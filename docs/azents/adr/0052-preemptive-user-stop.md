---
title: "ADR-0052: User Stop Uses Preemptive Interrupt and REST Control Boundary"
created: 2026-06-07
tags: [architecture, backend, frontend, api, chat, engine]
---

# ADR-0052: User Stop Uses Preemptive Interrupt and REST Control Boundary
...
ADR-0051 moved chat message/edit/command writes to REST commit boundary and left only stop request as WebSocket control. As a result, WebSocket handled both live subscription and stop input. Moving stop to REST makes WebSocket a server-to-client live projection-only channel.
...
### ADR-0052-D1. User stop is a preemptive interrupt over current run
...
### ADR-0052-D2. User stop in idle state is durable no-op without ack
...
### ADR-0052-D3. User stop during LLM call/streaming cancels immediately and durably stores only assistant text
...
### ADR-0052-D4. User stop during tool calling sends cancel signal fire-and-forget
...
### ADR-0052-D5. Tool calling stop fills unresolved active tool calls with cancelled results
...
### ADR-0052-D6. Streaming tool partial persistence is out of scope
...
### ADR-0052-D7. Separate terminal meaning of user stop and shutdown/handover stop
...
### ADR-0052-D8. User stop interruption is delivered to model input as user-role synthetic control event
...
### ADR-0052-D9. Stop button uses REST endpoint
...
### ADR-0052-D10. Chat WebSocket becomes live subscription-only channel
...
- [ADR-0051: Chat writes use REST commit boundary](./0051-rest-chat-write-boundary.md)
