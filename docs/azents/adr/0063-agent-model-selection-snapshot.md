---
title: "ADR-0063: Agent Model Selection Stores Catalog Snapshot Directly Without ModelConfig"
created: 2026-06-16
tags: [architecture, backend, api, frontend, engine]
---

# ADR-0063: Agent Model Selection Stores Catalog Snapshot Directly Without ModelConfig

## Context

ADR-0031 removed Agent runtime dependency on static `LLMModel` / `LLMProviderModel` catalog and introduced workspace-level `ModelConfig` alias/preset as Agent model selection contract. This structure provided dynamic model listing and workspace default preset, but actual product usage still had these problems:
...
### ADR-0063-D1. Remove `ModelConfig` entity
...
### ADR-0063-D2. Agent stores main/lightweight model selection snapshots directly
...
### ADR-0063-D3. Advanced model parameters move to Agent settings
...
### ADR-0063-D4. Workspace stores default model snapshot
...
### ADR-0063-D5. Workspace default is submit-time copy source, not runtime inheritance
...
### ADR-0063-D6. Define bootstrap rule for workspace default main model
...
### ADR-0063-D7. Remove subagent model inherit mode
...
### ADR-0063-D8. Do not keep legacy compatibility path
...
- [ADR-0031: Migrate Agent model selection to dynamic ModelConfig structure](./0031-dynamic-llm-model-configs.md)
