---
title: "ADR-0181: Suppress Unread Indicators While Sessions Run"
created: 2026-07-21
tags: [frontend, chat, ux]
---

# ADR-0181: Suppress Unread Indicators While Sessions Run

## Context

ADR-0174 established a durable Session-shared unread boundary for terminal Run results. A Session can begin a newer Run before an older terminal result is reviewed, so the durable unread boundary and `run_state = running` can coexist.
...
This supersedes only the Agent rail presentation aspect of ADR-0174. ADR-0174 remains the source of truth for durable shared unread-boundary semantics.
