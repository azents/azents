---
title: "Resolve User Message Profiles During Buffer Preparation"
created: 2026-07-12
tags: [architecture, agent, backend, engine, frontend, session, historical-reconstruction, migration]
...
# message-260712/ADR: Resolve User Message Profiles During Buffer Preparation

## Context

[time-260710/ADR](./time-260710-time-target-resolution.md) resolves a requested model target when an AgentRun starts, and [atomic-260710/ADR](./atomic-260710-atomic-profile-activation.md) updates the AgentSession last-used profile as part of run activation. Under [drain-260712/ADR](./drain-260712-drain-input-buffers-before-turn-start.md), input-buffer draining is now a preparation stage that completes before the next turn starts. Keeping label resolution in SessionRunner run preparation would preserve an unnecessary coupling between queued message semantics and turn creation.
...
This decision supersedes [time-260710/ADR](./time-260710-time-target-resolution.md) for normal buffered user-message resolution and supersedes [atomic-260710/ADR](./atomic-260710-atomic-profile-activation.md) where it delays the corresponding session profile update until AgentRun activation. [inference-260711/ADR](./inference-260711-inference-provenance.md) continues to apply to actual execution provenance: AgentRun remains its owner, while the event stores only the configuration applied during message preparation.
...
- [time-260710/ADR: Resolve Prompt Model Targets at Run Time](./time-260710-time-target-resolution.md)
- [atomic-260710/ADR: Atomically Activate the Resolved Run and Session Profile](./atomic-260710-atomic-profile-activation.md)
- [inference-260711/ADR: Keep Inference Provenance Run-Owned](./inference-260711-inference-provenance.md)
...
- Historical source filename: `0126-resolve-user-message-profile-during-buffer-preparation.md`
