---
title: "Chat Action Messages"
created: 2026-06-30
tags: [architecture, frontend, backend, chat, engine, historical-reconstruction, migration]
...
---
# chat-260630/ADR: Chat Action Messages
...
### chat-260630/ADR-D1. Introduce Chat Action Messages
...
### chat-260630/ADR-D2. Define `ChatAction` as a discriminated union
...
### chat-260630/ADR-D3. Categorize actions into Command Actions and Turn Actions
...
### chat-260630/ADR-D4. Generalize command handling around `CommandAction`
...
### chat-260630/ADR-D5. Treat Goal as a create-only Turn Action
...
### chat-260630/ADR-D6. Prefer failure handling over hard client-side availability blocking
...
### chat-260630/ADR-D7. Use an action chip inside the chat input box
...
### chat-260630/ADR-D8. Queue Turn Actions with a single `action_message` input-buffer kind
...
### chat-260630/ADR-D9. Preserve processed Turn Actions as `action_message` durable events
...
### chat-260630/ADR-D10. Treat `action_message` input-buffer items as turn barriers
...
### chat-260630/ADR-D11. Apply Goal Turn Action side effects during run-loop processing
...
### chat-260630/ADR-D12. Lower Turn Actions through action-specific handlers, not as plain user text
...
### chat-260630/ADR-D13. Consume failed Turn Actions and report with `system_error`
...
### chat-260630/ADR-D14. Record successful Goal creation with `action_message` and `goal_updated`
...
### chat-260630/ADR-D15. Reject Goal create only when an unfinished Goal exists

A Goal Turn Action creates a new active Goal only when no unfinished Goal exists.

Unfinished means a Goal whose status is `active`, `paused`, or `blocked`. If an unfinished Goal exists when the run loop processes a Goal create action, the action fails, is consumed, and reports a user-visible `system_error` as described in chat-260630/ADR-D13.
...
### chat-260630/ADR-D16. Render pending and durable action messages as input-box-like cards
...
### chat-260630/ADR-D17. Replace separate composer write endpoints with `/inputs`
...
### chat-260630/ADR-D18. Use optional `action`, string `message`, and action-specific validation
...
### chat-260630/ADR-D19. Expose composer actions through `/actions`
...
### chat-260630/ADR-D20. Move selected actions into composer chip state
...
### chat-260630/ADR-D21. Persist selected action in composer drafts
...
### chat-260630/ADR-D22. Use action policy for deterministic local validation
...
### chat-260630/ADR-D23. Keep Goal create visible with warning when an unfinished Goal appears to exist
...
The client may display this warning in slash search and selected action UI, but it must not treat the hint as an authoritative acceptance decision. `/inputs` remains the final validation boundary, and Goal create may still fail during run-loop processing according to chat-260630/ADR-D15 and chat-260630/ADR-D13.

### chat-260630/ADR-D24. Ship the composer action migration as one atomic PR
...
- Historical source filename: `0086-chat-action-messages.md`
