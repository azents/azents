---
title: "ADR-0066: Introduce credential provider model and admin password reset token"
created: 2026-06-18
tags: [architecture, backend, api, security]
---

# ADR-0066: Introduce credential provider model and admin password reset token

## Context

ADR-0065 separated new signup authority from email delivery so SMTP/SES is not mandatory prerequisite for open-source/self-host transition. New signup is controlled by signup token, and admin can manually deliver signup link even without SMTP.
...
### ADR-0066-D1. Login methods are modeled as CredentialProvider
...
### ADR-0066-D2. Internal CredentialSummary is rich, while API has purpose-specific projections
...
### ADR-0066-D3. Verified email is valid credential only when SMTP is configured
...
### ADR-0066-D4. Credential deletion must preserve at least one valid credential invariant
...
### ADR-0066-D5. Allow recovery-required state from SMTP disabled and recover by admin reset
...
### ADR-0066-D6. SMTP-less password recovery is provided by user_id-bound admin reset token
...
### ADR-0066-D7. Password reset token only recovers password credential and does not automatically log in
