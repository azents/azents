---
title: "Goal Continuation Uses Idle Hook and Input Buffer"
created: 2026-06-15
...
# goal-260615/ADR: Goal Continuation Uses Idle Hook and Input Buffer

## Context

[goal-260613/ADR](./goal-260613-goal-pursuing.md) decided that Goal is owned at `AgentSession` scope. [input-260615/ADR](./input-260615-input-control-plane-clean-migration.md) organized source of truth for model-visible payload entering session runner into `input_buffers` and reduced broker to control plane responsible only for wake-up and stop signal.
...
### goal-260615/ADR-D1. Add Goal continuation input buffer kind
...
### goal-260615/ADR-D2. Goal continuation promotes to separate durable event kind
...
### goal-260615/ADR-D3. LLM lower role uses existing user role
...
### goal-260615/ADR-D4. Add idle continuation lifecycle hook
...
### goal-260615/ADR-D5. `on_session_idle` returns zero or more continuation inputs
...
### goal-260615/ADR-D6. Worker/runtime stores hook result into input buffer and wakes up
...
### goal-260615/ADR-D7. GoalToolkit is the first `on_session_idle` provider
...
- Broker keeps only wake-up/signal role from [input-260615/ADR](./input-260615-input-control-plane-clean-migration.md).
...
Rejected. After [input-260615/ADR](./input-260615-input-control-plane-clean-migration.md), input buffer is source of truth for session runner payload, so continuation should use the same buffer taxonomy.
...
- [goal-260613/ADR: Goal pursuing uses session-scoped ownership](./goal-260613-goal-pursuing.md)
- [input-260615/ADR: Separate input payload and control action with DB source of truth](./input-260615-input-control-plane-clean-migration.md)

## Migration provenance

- Historical source filename: `0062-goal-continuation-idle-hook.md`
