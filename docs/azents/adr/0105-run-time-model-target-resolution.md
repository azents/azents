---
title: "ADR-0105: Resolve Prompt Model Targets at Run Time"
created: 2026-07-10
tags: [architecture, agent, backend, engine, routing]
---

# ADR-0105: Resolve Prompt Model Targets at Run Time

## Context

ADR-0100 introduced Agent-owned label-based model targets, and ADR-0103 made a prompt's requested target a FIFO run boundary. A target can either be resolved to a model snapshot when the server accepts the prompt or remain a durable routing intent until its `AgentRun` starts.
...
Runs without an explicit target inherit the AgentSession's last-used target under ADR-0106. The Agent default target is used only when that session has no last-used target. Neither source is a fallback for an explicitly requested or inherited target that fails resolution.
