---
title: "ADR-0061: Separate Input Payload and Control Action with DB Source of Truth"
created: 2026-06-15
tags: [architecture, backend, engine, api, chat]
---

# ADR-0061: Separate Input Payload and Control Action with DB Source of Truth

## Context

ADR-0034 decided to store user chat input in the `input_buffers` table and promote it to durable event at model-call boundary. ADR-0051 moved Web chat writes to REST commit boundary and defined REST success as input buffer commit.
...
### ADR-0061-D1. `input_buffers` is the source of truth for session runner input payload
...
### ADR-0061-D2. Input buffer item has promotion/rendering taxonomy through `kind`
...
### ADR-0061-D3. Edit is idle-only history rewrite command
...
### ADR-0061-D4. Background completion is stored as separate event kind and uses user role for model input
...
### ADR-0061-D5. Command is idle-only pending command, not input buffer
...
### ADR-0061-D6. Stop is DB-backed running-only interrupt intent
...
### ADR-0061-D7. Broker ingress keeps only wake-up and stop signal
...
- [ADR-0052: User stop uses preemptive interrupt and REST control boundary](./0052-preemptive-user-stop.md)
