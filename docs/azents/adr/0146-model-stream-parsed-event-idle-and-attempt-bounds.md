---
title: "ADR-0146: Bound Model Streams by Parsed-Event Idle and Absolute Attempt Time"
created: 2026-07-15
tags: [architecture, backend, engine, streaming, reliability]
---

# ADR-0146: Bound Model Streams by Parsed-Event Idle and Absolute Attempt Time
...
Model-stream timeouts use the existing failed-run retry policy rather than a timeout-specific retry budget. The initial policy permits up to 10 retries after the initial attempt, with the existing durable retry state, exponential backoff, worker-handover recovery, live retry projection, and user stop control. The current `failed_attempt_count >= max_retries` implementation is an off-by-one drift from ADR-0084 and must be corrected before timeout failures rely on the policy.
...
- [ADR-0052: Preemptive User Stop with In-Flight Run Finalization](./0052-preemptive-user-stop.md)
- [ADR-0084: Failed Run Errors Use One Bounded Automatic Retry](./0084-failed-run-error-retry.md)
