...
### ADR-0165-D1. Preserve safe provider-attributed failure details
...
### ADR-0165-D2. Normalize provider failures into one closed Engine contract
...
### ADR-0165-D3. Use one logical retry owner for each model operation
...
### ADR-0165-D4. Give every provider-attributed failure the complete retry budget
...
### ADR-0165-D5. Make automatic compaction part of the model-turn retry lifecycle
...
### ADR-0165-D6. Preserve provider attribution through retry and terminal presentation
...
### ADR-0165-D7. Keep User Stop terminal and non-replayable
...
This supersedes ADR-0084 only where Stop during failed-run backoff finalizes the latest provider failure instead of preserving the user-selected stopped outcome.

### ADR-0165-D8. Apply the shared policy to automatic title generation without failing the Run
...
### ADR-0165-D9. Emit safe structured provider-attempt telemetry
...
### ADR-0165-D10. Ship one coordinated behavior cutover without stopped-Run schema
...
ADR-0157 remains authoritative for cancellation, watchdog ownership, terminal completion requirements, transport cleanup, and the prohibition on raw SDK/provider payload propagation. This ADR narrows only its fixed-message treatment by permitting bounded redacted provider-authored scalar error fields inside the new typed contract.

ADR-0145 remains authoritative for explicit Responses terminal completion and model-turn-scoped durable retry state. This ADR extends that retry lifecycle to preserve typed provider failures consistently across supported adapters and automatic context preparation.

ADR-0084 remains authoritative for failed-run retry storage, backoff, handover, and terminal failed-run history except for the User Stop correction in ADR-0165-D7 and the complete-budget provider policy in ADR-0165-D4.
