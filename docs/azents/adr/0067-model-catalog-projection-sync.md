---
title: "ADR-0067: Model Catalog Projection and Sync"
created: 2026-06-20
tags: [architecture, backend, frontend, engine]
---

# ADR-0067: Model Catalog Projection and Sync
...
### ADR-0067-D1. Separate system catalog and integration catalog
...
### ADR-0067-D2. Do not call external catalog/provider listing from normal read path
...
### ADR-0067-D3. Full sync is the sync unit
...
### ADR-0067-D4. System catalog sync is periodic/admin only and assumes scheduler infrastructure
...
### ADR-0067-D5. Integration catalog sync is explicit and throttled
...
### ADR-0067-D6. User credential failures are domain state, not unhandled server errors
...
### ADR-0067-D7. Model picker uses integration-first modal with infinite scroll
...
### ADR-0067-D8. Use canonical projection layer
...
### ADR-0067-D9. LiteLLM catalog can be current projection source, but not permanent platform source of truth
...
### ADR-0067-D10. Provider x publisher matters for runtime projection
...
### ADR-0067-D11. Bedrock/Vertex integration projection uses provider availability plus target projection
...
### ADR-0067-D12. OpenAI/Anthropic provider APIs are not used yet
...
### ADR-0067-D13. Preserve agent model selection snapshot semantics
...
### ADR-0067-D14. Remove models.dev from catalog source path
...
### ADR-0067-D15. Store LiteLLM source snapshots separately from projections
...
### ADR-0067-D16. Keep only current snapshot and latest attempt state
...
### ADR-0067-D17. Initial integration sync runs after create/update in background
...
### ADR-0067-D18. Start with automatic exposure and hygiene filters, not allow policy
...
### ADR-0067-D19. Bedrock and Vertex selectable matching is exact-only in v1
...
- [ADR-0063: Agent Model Selection Stores Catalog Snapshot Directly Without ModelConfig](./0063-agent-model-selection-snapshot.md)
- [ADR-0030: Organize LLM model catalog source](./0030-llm-model-catalog-source.md)
