...
This run-scoped VFS lifecycle is separate from the existing filesystem Skill `latest` and `active` lifecycle. Filesystem Skill behavior remains governed by ADR-0087.
...
SkillAction preparation uses the same run projection and snapshots the loaded body and required metadata into durable model input as required by ADR-0130.
...
- Filesystem Skill failures continue to follow ADR-0087 and do not remove valid VFS entries.
...
This ADR extends ADR-0087 with an additional locator and resource system for Azents-managed files. ADR-0087 remains authoritative for Agent and Project filesystem Skill sources and their session projection lifecycle.

This ADR supersedes ADR-0087 only where ADR-0087 limits non-filesystem platform Skills to a future unspecified source and projects only `SKILL.md` content. It does not change existing filesystem source conventions or path-based Skill behavior.

ADR-0130 continues to govern SkillAction preparation. For an `azents://` Skill, the selected body and metadata come from the immutable VFS projection owned by that run.

## References

- [ADR-0087: Filesystem Skill Projection Revisions](./0087-filesystem-skill-projection-revisions.md)
- [ADR-0130: Treat Skill Actions as Model-Producing Preparation](./0130-treat-skill-actions-as-model-producing-preparation.md)
- [ADR-0046: File Media Resource Lifecycle](./0046-file-media-resource-lifecycle.md)
