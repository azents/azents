---
title: "ADR-0084: Failed-run Error Retry and Finalization"
created: 2026-06-27
tags: [architecture, backend, engine, worker, retry]
---
# ADR-0084: Failed-run Error Retry and Finalization
...
### ADR-0082-D1. Use a shared failed-run finalizer with RunExecutor-centered ownership
...
### ADR-0082-D2. Convert exceptions to structured failed attempts at the retry boundary
...
### ADR-0082-D3. Treat every run-stopping failure as failed-run finalization scope
...
### ADR-0082-D4. Store retry state on `agent_runs` and project it to live state
...
### ADR-0082-D5. Expose retry status as part of the live run state
...
### ADR-0082-D6. Apply one v1 retry policy to all run-stopping failures
...
### ADR-0082-D7. Resume retry after handover by honoring `next_retry_at`
...
### ADR-0082-D8. Stopping during retry promotes the latest error to final failed-run output
...
### ADR-0082-D9. Goal continuation runs only after successful run completion
...
### ADR-0082-D10. Extend `system_error` with failed-run metadata
...
- [ADR-0062: Goal Continuation Idle Hook](./0062-goal-continuation-idle-hook.md)
