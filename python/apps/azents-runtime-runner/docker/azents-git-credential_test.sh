...
result=$(GH_TOKEN="example_token_1" "$SCRIPT" get <<'EOF'
protocol=https
host=github.com
path=owner/repo.git
EOF
...
echo "$result" | grep -q "^password=example_token_1$" || fail "case1 missing/wrong password"
...
result=$(GH_TOKEN="example_token" "$SCRIPT" get <<'EOF'
protocol=https
host=example.com
path=owner/repo.git
EOF
...
result=$(GH_TOKEN="example_token" "$SCRIPT" store <<'EOF'
protocol=https
host=github.com
