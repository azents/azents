"""xAI OAuth session repository tests."""
...
async def _create_session(
...
) -> tuple[XaiOAuthSessionRepository, str]:
    ...
    workspace_result = await workspace_repo.create(
        session,
        WorkspaceCreate(
            name=f"xAI OAuth test {suffix}",
            handle=f"xai-oauth-{suffix}",
        ),
    )
    assert isinstance(workspace_result, Success)
    workspace_id = await workspace_repo.resolve_id(session, f"xai-oauth-{suffix}")
    assert workspace_id is not None
    user = await UserRepository().create(
        session,
        UserCreate(email=f"xai-oauth-{suffix}@example.com"),
    )
    repo = XaiOAuthSessionRepository(CredentialCipher(_TEST_KEY))
