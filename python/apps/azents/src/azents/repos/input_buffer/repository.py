"""InputBuffer repository implementation."""
...
class InputBufferRepository:
    ...
    async def create_with_id(
        ...
        *,
        ...
        """Create one InputBuffer with a preallocated durable request target ID."""
        rdb = self._new_rdb(create)
        rdb.id = input_buffer_id
        session.add(rdb)
        await session.flush()
        return self._build(rdb)
        ...
        *,
        ...
        stmt = (
            ...
            .values(
                ...
                action=self._action_payload(create.action),
    ...
    def _build(self, rdb: RDBInputBuffer) -> InputBuffer:
        ...
        return InputBuffer(
            ...
            action=(
                None
                if rdb.action is None
                else _JSON_OBJECT_ADAPTER.validate_python(rdb.action)
            ),
    ...
    def _new_rdb(self, create: InputBufferCreate) -> RDBInputBuffer:
        """Build one ORM row without assigning its generated primary key."""
        return RDBInputBuffer(
            session_id=create.session_id,
            kind=create.kind,
            requested_model_target_label=create.requested_model_target_label,
            requested_reasoning_effort=create.requested_reasoning_effort,
    ...
    def _action_payload(
        ...
        """Widen validated recursive JSON values for the ORM JSONB field."""
        if action is None:
            return None
