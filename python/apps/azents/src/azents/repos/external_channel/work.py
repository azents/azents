...
# Bound one-time recovery to attempts observed before the authority-fix rollout.
_INITIAL_DISCORD_DELIVERY_AUTHORITY_REPAIR_CUTOFF = datetime.datetime(
    2026,
    7,
    29,
    2,
...
class ExternalChannelWorkRepository:
    ...
    async def restore_initial_discord_deliveries(
        self,
        session: AsyncSession,
        *,
        binding_id: str,
    ) -> int:
        """Restore exact initial intents rejected before any provider attempt."""
        binding = await session.scalar(
            sa.select(RDBExternalChannelBinding)
            .where(
                RDBExternalChannelBinding.id == binding_id,
        ...
        repairable = (
            ...
            RDBExternalChannelDeliveryAttempt.provider_message_key.is_(None),
            ...
            < _INITIAL_DISCORD_DELIVERY_AUTHORITY_REPAIR_CUTOFF,
            RDBExternalChannelDeliveryAttempt.completed_at
            < _INITIAL_DISCORD_DELIVERY_AUTHORITY_REPAIR_CUTOFF,
        )
        session_link = await session.scalar(
            sa.select(RDBExternalChannelDeliveryAttempt)
            .where(
                ...
                RDBExternalChannelDeliveryAttempt.part_ordinal == 0,
        ...
        progress_attempts = list(
            await session.scalars(
                ...
                .where(
                    ...
                    RDBExternalChannelDeliveryAttempt.origin_type
                    == ExternalChannelDeliveryOriginType.MANAGER_OPERATION,
                    RDBExternalChannelDeliveryAttempt.origin_id == work.id,
                    RDBExternalChannelDeliveryAttempt.channel_action_id.is_(None),
                    RDBExternalChannelDeliveryAttempt.operation
                    == ExternalChannelDeliveryOperation.PROGRESS_CREATE,
                ...
                .order_by(RDBExternalChannelWorkProjectionPart.part_ordinal)
                .with_for_update(of=RDBExternalChannelDeliveryAttempt)
            )
        )
        exact_attempts = [session_link] if session_link is not None else []
        exact_attempts.extend(progress_attempts)
