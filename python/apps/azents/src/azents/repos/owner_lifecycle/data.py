"""Owner lifecycle repository data models."""
...
class OwnerLifecycleJob(BaseModel):
    """Durable owner-lifecycle job."""

    id: str = Field(description="Owner lifecycle job ID")
    kind: OwnerLifecycleKind = Field(description="Lifecycle operation kind")
    user_id: str = Field(description="Target User ID")
    workspace_id: str | None = Field(
        default=None,
        description="Workspace ID for membership archive jobs",
    )
    status: OwnerLifecycleStatus = Field(description="Job status")
