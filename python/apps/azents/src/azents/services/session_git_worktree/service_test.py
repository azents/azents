...
class TestSessionGitWorktreeService:
    ...
    async def test_attach_git_worktree_to_existing_session_registers_worktree_project(
        self,
        rdb_session_manager: SessionManager[AsyncSession],
    ) -> None:
        """Existing sessions can queue a Git worktree Project registration."""
        async with rdb_session_manager() as session:
            _, user_id, agent_id = await _create_agent_context(session, "attach")
        runner = _RunnerOperations()
        worktree_service = _service(rdb_session_manager, runner)
        input_service = _input_service(rdb_session_manager, worktree_service)
        ...
        result = await input_service.create_team_session_with_buffered_input(
            ...
            client_request_id="attach-existing",
        )
        assert isinstance(result, Success)
        session_id = result.value.agent_session.id
        async with rdb_session_manager() as session:
        ...
        assert runner.calls[-1]["source_project_path"] == "/workspace/agent/repo"

    async def test_pending_worktree_registrations_run_as_one_queue(
        self,
        rdb_session_manager: SessionManager[AsyncSession],
    ) -> None:
        """Multiple pending worktree attachments run as one sequential queue."""
        async with rdb_session_manager() as session:
            _, user_id, agent_id = await _create_agent_context(
                session,
                "attach-queue",
            )
        runner = _RunnerOperations()
        ...
        result = await input_service.create_team_session_with_buffered_input(
            ...
            client_request_id="attach-queue",
        )
        assert isinstance(result, Success)
        session_id = result.value.agent_session.id
        async with rdb_session_manager() as session:
        ...
        first = await worktree_service.attach_git_worktree_to_session(
            agent_id=agent_id,
            session_id=session_id,
            user_id=user_id,
            source_project_path="/workspace/agent/repo-a",
            starting_ref="main",
        )
        second = await worktree_service.attach_git_worktree_to_session(
            agent_id=agent_id,
            session_id=session_id,
            user_id=user_id,
            source_project_path="/workspace/agent/repo-b",
            starting_ref="feature",
        )
        ...
        assert [call["source_project_path"] for call in create_calls] == [
            "/workspace/agent/repo-a",
            "/workspace/agent/repo-b",
        ]
        assert [project.path for project in projects] == [
            "/workspace/agent/app",
    ...
    async def test_attach_rejects_running_initialization_queue(
        self,
        rdb_session_manager: SessionManager[AsyncSession],
    ) -> None:
        """Worktree attachment waits for the current initialization run to settle."""
        runner = _RunnerOperations()
        (
            worktree_service,
            user_id,
        ...
        async with rdb_session_manager() as session:
            ...
            assert initialization is not None
            await SessionInitializationRepository().update_initialization_status(
                session,
                initialization_id=initialization.id,
        ...
        assert isinstance(attach_result, Failure)
        assert isinstance(attach_result.error, GitWorktreeAttachUnavailable)

    async def test_second_worktree_registration_skips_ready_first_worktree(
        self,
        rdb_session_manager: SessionManager[AsyncSession],
    ) -> None:
        """Sequential worktree registration runs only unfinished allocations."""
        runner = _RunnerOperations()
        (
            worktree_service,
            user_id,
        ...
        assert isinstance(attach_result, Success)
        async with rdb_session_manager() as session:
            queued_initialization = (
                await SessionInitializationRepository().get_by_session_id(
                    session,
                    session_id=session_id,
                )
            )
        assert queued_initialization is not None
        assert queued_initialization.status is SessionInitializationStatus.PENDING
        assert queued_initialization.completed_at is None
    ...
    async def test_cleanup_removes_all_session_worktrees(
        self,
        rdb_session_manager: SessionManager[AsyncSession],
    ) -> None:
        """Cleanup covers every Azents-owned worktree in the session."""
        runner = _RunnerOperations()
        (
            worktree_service,
            user_id,
