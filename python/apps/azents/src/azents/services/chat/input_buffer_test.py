...
class TestChatSessionInputBuffer:
    ...
    async def test_list_live_events_projects_compacting_as_one_live_operation(
        ...
        assert result.value.run.operation.operation_id == run.id
        assert result.value.run.operation.status == "running"
        assert result.value.run.recovery is None
        assert result.value.session_run_state == AgentSessionRunState.RUNNING

    async def test_list_live_events_includes_recoverable_stopped_run(
        self,
        rdb_session_manager: SessionManager[AsyncSession],
    ) -> None:
        """An idle session retains its latest recoverable stopped Run."""
        async with rdb_session_manager() as session:
            session_id, user_id, buffer_id = await _create_session_with_buffer(
                session,
                handle="chat-live-stopped-recovery",
                slug="chat-live-stopped-recovery",
            )
            await InputBufferRepository().delete_by_session_and_id(
                session,
                session_id,
            ...
            stopped_at = now + datetime.timedelta(seconds=1)
            run = await run_repository.mark_stopped_with_recovery(
                session,
                run.id,
        ...
        assert result.value.run is not None
        assert result.value.run.run_id == run.id
        assert result.value.run.status == AgentRunStatus.STOPPED
        assert result.value.run.operation is None
        assert result.value.run.retry is None
        assert result.value.run.recovery is not None
        assert result.value.run.recovery.user_message == (
            "Model provider error: quota exhausted"
        )
        assert result.value.run.recovery.operation == "sampling"
        assert result.value.run.recovery.source_run_id == run.id
        assert result.value.run.recovery.stopped_at == stopped_at.isoformat()

    async def test_pending_input_replaces_recoverable_stopped_run_projection(
        self,
        rdb_session_manager: SessionManager[AsyncSession],
    ) -> None:
        """A new pending user input immediately dismisses stopped recovery."""
        async with rdb_session_manager() as session:
            session_id, user_id, buffer_id = await _create_session_with_buffer(
                session,
                handle="chat-live-stopped-replaced",
                slug="chat-live-stopped-replaced",
            )
            now = datetime.datetime.now(datetime.UTC)
            await AgentSessionRepository().set_inference_state(
                session,
            ...
            stopped_at = now + datetime.timedelta(seconds=1)
            await run_repository.mark_stopped_with_recovery(
                session,
                run.id,
                recovery_state=RunRecoveryState(
                    kind="provider_failure",
