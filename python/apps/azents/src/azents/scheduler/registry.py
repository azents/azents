...
async def file_lifecycle_cleanup_handler(context: TaskContext) -> TaskResult:
    """Run attachment, artifact, and ModelFile lifecycle cleanup."""
    ...
    return TaskResult(
        summary={
            ...
            "attempt_started_at": context.attempt_started_at.isoformat(),
            "manual_triggered": context.manual_triggered,
            "exchange_files_expired": summary.exchange_files_expired,
            "artifacts_expired": summary.artifacts_expired,
            "model_files_changed": summary.model_files_changed,
        }
    )
...
FILE_LIFECYCLE_CLEANUP_TASK = ScheduledTaskDefinition(
    ...
    description="Expire Exchange files, Artifacts, and ModelFiles in background.",
    interval=datetime.timedelta(minutes=5),
    timeout=datetime.timedelta(minutes=2),
    retry_policy=RetryPolicy(
        kind="bounded_backoff",
        min_delay=datetime.timedelta(minutes=1),
        max_delay=datetime.timedelta(minutes=15),
    ),
