...
async def external_account_oauth_cleanup_handler(
    context: TaskContext,
) -> TaskResult:
    """Delete one bounded batch of retained OAuth attempts."""
    service = await context.container.solve(ExternalAccountOAuthAttemptService)
...
EXTERNAL_ACCOUNT_OAUTH_CLEANUP_TASK = ScheduledTaskDefinition(
    key="external_account_oauth_cleanup",
    description="Delete provider OAuth attempts outside their retention window.",
    ...
    handler=external_account_oauth_cleanup_handler,
    enabled_by_default=True,
)
