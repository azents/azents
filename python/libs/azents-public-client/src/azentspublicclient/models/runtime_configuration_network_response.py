...
class RuntimeConfigurationNetworkResponse(BaseModel):
    ...
    domain_mode: Optional[StrictStr]
    protocol_summary: StrictStr
    https_inspection: StrictBool
    enforcement_status: RuntimeConfigurationStatus
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["mode", "domain_mode", "protocol_summary", "https_inspection", "enforcement_status"]

    @field_validator('protocol_summary')
    def protocol_summary_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['direct_ip', 'http_https_websocket', 'platform_only']):
            raise ValueError("must be one of enum values ('direct_ip', 'http_https_websocket', 'platform_only')")
        return value
