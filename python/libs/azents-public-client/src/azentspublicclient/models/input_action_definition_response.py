...
class InputActionDefinitionResponse(BaseModel):
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="Action definition ID")
    ...
    def category_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['command', 'turn']):
            raise ValueError("must be one of enum values ('command', 'turn')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of availability_hint
        if self.availability_hint:
            _dict['availability_hint'] = self.availability_hint.to_dict()
        # puts key-value pairs in additional_properties in the top level
        if self.additional_properties is not None:
            for _key, _value in self.additional_properties.items():
                _dict[_key] = _value

        # set to None if availability_hint (nullable) is None
